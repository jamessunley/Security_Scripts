...
class DependencyCheck:
    ...
    def Scan(locationToScan, supressionFile, outputLocation, exclusionsList, dependencyCheckLocation, os):
            print("Starting Scan")
            print("Folder to scan: " + locationToScan)
            print("Report will be output to: " + outputLocation)
            stream = os.popen(dependencyCheckLocation + ' --project `"projectId`" --scan `"locationToScan`" --suppression `"supressionFile`" exclusionsList --out `"outputLocation`" -f HTML')
            output = stream.read()
            print(output)

    exclusionsList = Exclusions()
    Scan(locationToScan, supressionFile, outputLocation, exclusionsList, dependencyCheckLocation, os)